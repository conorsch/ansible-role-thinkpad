...
def main():
    ...
    regex = re.compile(r'^(?P<display_name>\w+) connected\s?((?P<primary>primary)?)\s?(?P<resolution_as_string>(?P<horizontal_resolution>\d+)x(?P<vertical_resolution>\d+)).*')
    try:
    ...
    if matches.get('primary') == 'primary':
        matches['primary'] = True
    else:
        matches['primary'] = False
    module.exit_json(changed=False, ansible_facts=matches)
